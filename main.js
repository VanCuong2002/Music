...
const app = {
    ...
    themeLists: JSON.parse(
        localStorage.getItem(THEME_LIST_STORAGE_KEY) || "[]"
    ), // List theme image to render to view
    ...
    renderSong() {
        ...
        songLists.forEach((songList, songIndex) => {
            songList.innerHTML = app.html`${app.songs.map(function (
                song,
                index
            ) {
                return app.html`
    ...
    renderSpecialPlaylist() {
        specialPlaylistLists.forEach(
            (specialPlaylistList, playlistListIndex) => {
    ...
    handleEvents: function () {
        ...
        const newPlaylistMove = $(
            ".container__header-actions.new-playlist--move"
        );
        ...
        appContainers.forEach((appContainer) => {
            appContainer.onscroll = function () {
                const scrollTop =
                    appContainer.scrollY || appContainer.scrollTop;
                if (scrollTop > 5) {
            ...
            if (audio.duration) {
                if (!_this.isSeeking) {
                    ...
                    trackTimes.forEach((trackTime) => {
                        trackTime.innerHTML = _this.audioCalTime(
                            audio.currentTime
                        );
                    });
                    ...
                    if (
                        ...
                        listDurationTime.innerHTML =
                            _this.durationList[_this.currentPlaylist][
                                _this.currentIndex
                            ];
        ...
        player.onclick = (e) => {
            ...
            const popDownBtn = e.target.closest(
                ".popup__action-btn.btn--pop-down"
            );
            if (
        ...
        songLists.forEach((songList) => {
            songList.onclick = function (e) {
                ...
                const songNode = e.target.closest(
                    ".playlist__list-song:not(.active)"
                );
        ...
        modalTheme.onclick = (e) => {
            const themeContainer = e.target.closest(
                ".modal-theme .modal-container"
            );
            if (themeContainer) {
                e.stopPropagation();
            } else {
        ...
        listThemes.forEach((listTheme, themeIndex) => {
            listTheme.onclick = (e) => {
                ...
                const previewBtn = e.target.closest(
                    ".theme__actions-btn.btn--preview"
                );
        ...
        sidebarNavItems.forEach((sidebarNavItem, index) => {
            sidebarNavItem.onclick = (e) => {
                ...
                $(".sidebar__nav .sidebar__nav-item.active").classList.remove(
                    "active"
                );
        ...
        function newPlaylistSlideShow(step) {
            ...
            newPlaylistMove.onclick = (e) => {
                const prevBtn = e.target.closest(
                    ".move-btn--new-playlist.btn--prev"
                );
                const nextBtn = e.target.closest(
                    ".move-btn--new-playlist.btn--next"
                );
                if (nextBtn) {
                    _this.showSlides(
                        step,
        ...
        function favArtistSlideShow(step) {
            ...
            favArtistMove.onclick = (e) => {
                const prevBtn = e.target.closest(
                    ".move-btn--fav-artist.btn--prev"
                );
                const nextBtn = e.target.closest(
                    ".move-btn--fav-artist.btn--next"
                );
                if (nextBtn) {
                    _this.showSlides(
                        step,
        ...
        storyActions.forEach((storyAction) => {
            storyAction.onclick = (e) => {
                ...
                if (btnComment) {
                    showNotificationToast(
                        "Tính năng hiện tại chưa được cập nhật, bạn vui lòng thông cảm!"
                    );
                }
            };
        });
    ...
    setPlayerInfoWidth() {
        ...
        const playerSongTitles = Array.from(
            $$(".player__song-title.info__title")
        );
        playerSongTitles.forEach((playerSongTitle) => {
            playerSongTitle.style.width =
                songAnimateTitles[0].offsetWidth / 2 + "px";
        });
    ...
    loadConfig: function () {
        ...
        audio.volume =
            this.config.currentVolume == 0
                ? 0
                : this.config.currentVolume / 100 || 1;
        if (!audio.volume) {
            volumeIcons.forEach((volumeIcon) => {
    ...
    loadThemeBg(themeListIndex, currentTheme) {
        const currentThemeColor =
            this.themes[themeListIndex][currentTheme].colors;
